...
def update():
    ...
    while video_pattern.search(html, video_pos):
        ...
        str_html = str_html.replace(video_str.group(), 'http://www.dilidili.wang' + video_str.group())
        video_pos = video_str.span()[1]

    while image_pattern.search(html, image_pos):
        image_str = image_pattern.search(html, image_pos)
        str_html = str_html.replace(image_str.group(), 'http://www.dilidili.wang' + image_str.group())
